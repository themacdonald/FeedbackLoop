...
class WorkRuntime:
    ...
    def authorize(self, work: Work, actor_id: str) -> None:
        ...
        self._require_actor(work, actor_id)
    ...
    def fail(self, work: Work, actor_id: str, reason: str) -> None:
        self._require_transition(work, WorkState.FAILED)
        self._require_actor(work, actor_id)
        if not reason.strip():
            raise ValueError("A failure reason is required.")
        self._transition(work, WorkState.FAILED, actor_id, "failed", {"reason": reason})

    def cancel(self, work: Work, actor_id: str, reason: str) -> None:
        self._require_transition(work, WorkState.CANCELLED)
        self._require_actor(work, actor_id)
        if not reason.strip():
            raise ValueError("A cancellation reason is required.")
        self._transition(work, WorkState.CANCELLED, actor_id, "cancelled", {"reason": reason})

    def _require_actor(self, work: Work, actor_id: str) -> None:
        if not work.authority.permits(actor_id, work.capability.name):
    ...
    def _require_transition(self, work: Work, target: WorkState) -> None:
        if target not in self._TRANSITIONS[work.state]:
            raise ValueError(f"Invalid state transition: {work.state.value} -> {target.value}")

    def _transition(self, work: Work, target: WorkState, actor_id: str,
                    event_type: str, details: dict | None = None) -> None:
        previous = work.state
        work.state = target
        work.record_event(WorkEvent.create(
            event_type=event_type, work_id=work.work_id, actor_id=actor_id,
            from_state=previous.value, to_state=target.value, details=details,
        ))
